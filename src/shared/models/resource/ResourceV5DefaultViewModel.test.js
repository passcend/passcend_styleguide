...
describe("ResourceV5DefaultViewModel", () => {
  describe("::createFromEntity", () => {
    it("should create the right resourceViewModel given an entity", () => {
      ...
      const resourceViewModel = ResourceV5DefaultViewModel.createFromEntity(resourceDto);
      expect(resourceViewModel).toBeInstanceOf(ResourceV5DefaultViewModel);
      expect(resourceViewModel.id).toStrictEqual(resourceDto.id);
      expect(resourceViewModel.resource_type_id).toStrictEqual(TEST_RESOURCE_TYPE_V5_DEFAULT);
  ...
  describe("::getSchema", () => {
    it("schema must validate", () => {
      EntitySchema.validateSchema(ResourceV5DefaultViewModel.name, ResourceV5DefaultViewModel.getSchema());
    });

    it("schema not have 'id' field set as required in CREATE_MODE", () => {
      expect.assertions(1);

      const schema = ResourceV5DefaultViewModel.getSchema(ResourceViewModel.CREATE_MODE);
    ...
    it("schema have 'id' field set as required in EDIT_MODE", () => {
      ...
      const schema = ResourceV5DefaultViewModel.getSchema(ResourceViewModel.EDIT_MODE);
  ...
  describe("::resourceTypeSlug", () => {
    it("should return the right slug", () => {
      ...
      expect(ResourceV5DefaultViewModel.resourceTypeSlug).toStrictEqual("v5-default");
    });
  });
  ...
  describe("::canToggleDescription", () => {
    it("should return true", () => {
      ...
      const viewModel = new ResourceV5DefaultViewModel();
