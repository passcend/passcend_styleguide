...
describe("DisplayPasswordPoliciesAdministrationTeasing", () => {
  ...
  it('As CE AD I should see all details about the password policy', async() => {
    ...
    expect(page.helpBoxDescription).toBe("For more information about the password policy settings, checkout the dedicated page on the help website.");
    expect(page.helpBoxButton.textContent).toEqual("Read the documentation");
    expect(page.helpBoxButton.getAttribute("href")).toBe("https://passbolt.com/docs/admin/password-configuration/password-policy/");
  });
});
